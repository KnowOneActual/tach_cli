...
def test_cli_timer_run(mock_app):
...
@patch("tach_cli.cli.TachApp")
def test_cli_clock_run(mock_app):
...
@patch("tach_cli.cli.TachApp")
def test_cli_timer_profile_plus_flags(mock_app):
    ...
    with patch("tach_cli.cli.load_config") as mock_load:
        ...
        assert result.exit_code == 0
        mock_app.assert_called_once()
        assert mock_app.call_args[1]["spec"].total_seconds == 10 # Flag wins in current logic
